...
export function registerAutoImportProvider(
	context: vscode.ExtensionContext,
	index: StubIndex
): void {

	var provider: vscode.CodeActionProvider = {
		provideCodeActions(doc, _range, ctx) {
			...
			var maxSuggestions = vscode.workspace
				...
				.get<number>("maxImportSuggestions", 15);
...
function createAction(
	...
	if (existing) {
		...
		var newLine =
			"from " + modulePath + " import " + newNames.join(", ");

		edit.replace(
			doc.uri,
			new vscode.Range(
				new vscode.Position(existing.line, 0),
				new vscode.Position(existing.line, Number.MAX_SAFE_INTEGER)
			),
			newLine
		);
	} else {
